...
def query_synapse_for_folder_info(ref_path, data_type, folder_id_column_name, grant_id_column_name):
    
    syn = synapseclient.login()

    grant_table = syn.tableQuery(f'SELECT "grantNumber", "grantId" AS {grant_id_column_name} FROM syn21918972').asDataFrame().fillna("")
    
    folder_table = syn.tableQuery(f"SELECT id AS {folder_id_column_name}, name, projectId AS {grant_id_column_name}  FROM syn27210848 WHERE name='{data_type}' AND parentId=projectId").asDataFrame().fillna("")

    grant_folder_reference = grant_table.merge(folder_table, how='left', on=grant_id_column_name)
...
def get_folder_id_and_grant_id_from_csv(grant_folder_reference, ca_number, folder_id_column_name, grant_id_column_name):
    ...
    if folder_id_column_name in df.columns and grant_id_column_name in df.columns:
        ...
        print(f"Error: '{folder_id_column_name}' or '{grant_id_column_name}' column not found in the CSV file: {grant_folder_reference}")
        return None, None

def write_file_paths_to_csv(file_paths, output_file, file_suffix, grant_folder_reference, folder_id_column_name, grant_id_column_name):
    
    ref_name = "".join(["grant_folder_reference_", datetime.today().strftime('%Y-%m-%d'), ".csv"])
    base_path = os.path.dirname(file_paths[0])
    ref_path = os.path.join(base_path, ref_name)
    grant_folder_reference = query_synapse_for_folder_info(ref_path, data_type, folder_id_column_name, grant_id_column_name)
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        for file_path in file_paths:
            ...
            folder_id, grant_id = get_folder_id_and_grant_id_from_csv(grant_folder_reference, ca_number, folder_id_column_name, grant_id_column_name) 
...
def main(folder_path, output_csv_file, data_type):
    ...
    manifest_for_upload = write_file_paths_to_csv(file_paths, output_csv_file, data_type, file_suffix, folder_id_column_name, grant_id_column_name)
