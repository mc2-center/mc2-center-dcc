...
def get_args():
    ...
    parser.add_argument("-d", "--dataset_table",
                        type=str, default="syn21897968",
                        help=("Synapse ID to the datasets table. "
                              "(Default: syn21897968"))
    parser.add_argument("-p", "--pubs_table",
                        type=str, default="syn21868591",
                        help=("Synapse ID to the publications table. "
                              "(Default: syn21868591)"))
...
def main():
    ...
    datasets = (
        syn.tableQuery(
            f"SELECT datasetAlias, pubMedId FROM {args.dataset_table}")
        .asDataFrame()
        .explode('pubMedId')
    )
